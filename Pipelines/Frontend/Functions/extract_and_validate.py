...
def _validate_input_path(input_path: str) -> Path:
    ...
    if not path.is_file():
        raise ValueError(
            "Den valgte inputfil findes ikke. Gå tilbage og vælg en ny fil."
        )

    if path.suffix.lower() != ".pdf":
        raise ValueError(
            "Den valgte inputfil er ikke en PDF-fil. Gå tilbage og vælg en PDF-fil."
        )
...
def _is_valid_name_part(part: str) -> bool:
    ...
    # Initial, e.g. "B."
    if len(part) == 2 and part[0].isalpha() and part[0].isupper() and part[1] == ".":
        return True
    ...
    if "-" in part:
        ...
        return all(_is_valid_name_part(section) for section in sections)
...
def _is_expected_name(name: str) -> bool:
    ...
    return all(_is_valid_name_part(part) for part in parts)
...
def _validate_name_list(
    ...
    if required and not names:
        raise ValueError(f"Feltet '{field_name}' skal udfyldes.")

    warnings = []

    for name in names:
        if not _is_expected_name(name):
            warnings.append(f"{field_name}: '{name}'")
...
def _is_expected_addition(value: str) -> bool:
    ...
    return all(
        character.isalnum() or character in allowed_special_characters
...
def _validate_additions(
    ...
    for addition in additions:
        if not _is_expected_addition(addition):
            warnings.append(f"Andet der skal anonymiseres: '{addition}'")
...
def extract_and_validate(
    ...
    additions, addition_warnings = _validate_additions(args.additions)

    warnings = (
        child_warnings + parent_1_warnings + parent_2_warnings + addition_warnings
    )
